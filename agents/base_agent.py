...
class BaseSectionAgent(ABC):
    ...
    def _build_hyde_refinement_prompt(
        ...
        system = (
        "Tu es un analyste senior spécialisé en études de marché, avec 15 ans "
        "d'expérience dans la rédaction de rapports sectoriels (type Xerfi, "
        "McKinsey, Gartner, CB Insights).\n\n"
        "TÂCHE : à partir d'une réponse hypothétique brute, tu rédiges un "
        "paragraphe qui doit être indiscernable d'un extrait RÉEL tiré d'un "
        "rapport de marché professionnel publié. Ce paragraphe sert "
        "uniquement de vecteur de recherche par similarité (technique HyDE) "
        "dans une base de connaissances de rapports de marché : il ne sera "
        "jamais lu tel quel par un humain final, seule sa proximité "
        "sémantique avec de vrais extraits compte.\n\n"
        "EXIGENCES DE STYLE :\n"
        "- Vocabulaire sectoriel précis et terminologie technique du domaine "
        "concerné.\n"
        "- Chiffres, pourcentages, taux de croissance (CAGR), montants "
        "plausibles et cohérents avec le secteur (même approximatifs, ils "
        "doivent être réalistes).\n"
        "- Ton factuel, neutre, déclaratif — comme un constat établi, jamais "
        "une hypothèse ou une supposition.\n"
        "- Structure typique d'un rapport : contexte de marché, données "
        "chiffrées, tendance ou dynamique, acteurs ou segments si pertinent.\n\n"
        "INTERDICTIONS STRICTES :\n"
        "- Aucune première personne ('je', 'nous pensons', 'il est possible "
        "que').\n"
        "- Aucune formule de doute ou de conditionnel ('pourrait', "
        "'semblerait', 'on estime généralement').\n"
        "- Aucun méta-commentaire sur la tâche ('voici le paragraphe', "
        "'en tant qu'IA', 'd'après la question').\n"
        "- Aucun titre, aucune puce, aucun markdown : un seul paragraphe brut.\n"
        "- Ne jamais mentionner qu'il s'agit d'un texte généré ou hypothétique.\n\n"
        "FORMAT DE SORTIE : un unique paragraphe de 4 à 6 phrases, en français, "
        "sans guillemets, sans préambule, sans commentaire — uniquement le texte."
    )
        user = (
        f"## Question principale de la section ({self.SECTION_LABEL})\n"
        f"{question}\n\n"
        f"## Sous-requête à cibler\n"
        f"{sub_query.sub_query}\n\n"
        f"## Réponse hypothétique initiale (recherche web, à reformuler)\n"
        f"{sub_query.hyde_answer}\n\n"
        f"## Contexte projet\n"
        f"{self._format_project_info()}\n\n"
        f"## Exemple de style attendu (autre secteur, à titre d'illustration)\n"
        f"\"Le marché européen des solutions de gestion des identités "
        f"numériques a atteint 4,2 milliards d'euros en 2023, porté par une "
        f"croissance annuelle moyenne de 14,8 % sur la période 2020-2023. "
        f"Cette dynamique s'explique principalement par le durcissement des "
        f"réglementations sur la protection des données et l'essor de "
        f"l'authentification multifactorielle en entreprise. Les acteurs "
        f"historiques du secteur IAM (Identity and Access Management) font "
        f"face à une concurrence croissante de startups spécialisées dans la "
        f"biométrie comportementale, qui captent désormais près de 18 % des "
        f"nouveaux contrats signés en Europe de l'Ouest.\"\n\n"
        f"## Consigne finale\n"
        f"Réécris la réponse hypothétique ci-dessus en un paragraphe conforme "
        f"au style illustré, adapté au sujet et au sous-requête ci-dessus. "
        f"Réponds uniquement avec ce paragraphe, rien d'autre."
    )
        return system, user
    ...
    def _build_question_prompt(self, question: str, context: str) -> Tuple[str, str]:
        ...
        system = system = (
        f"Tu es un analyste senior en études de marché, spécialisé dans la "
        f"rédaction de la section '{self.SECTION_LABEL}' d'un rapport "
        f"professionnel destiné à un client.\n\n"
        f"RÈGLE ABSOLUE — ANCRAGE AU CONTEXTE :\n"
        f"- Tu réponds EXCLUSIVEMENT à partir des extraits fournis dans "
        f"'Contexte' et des 'Informations projet'.\n"
        f"- Tu n'utilises AUCUNE connaissance externe, même si tu la crois "
        f"exacte : seul ce qui est écrit dans le contexte fait foi.\n"
        f"- Si une information nécessaire est absente, insuffisante ou "
        f"contradictoire dans le contexte, dis-le explicitement dans le "
        f"corps de la réponse (ex. \"Le contexte ne précise pas...\"), sans "
        f"jamais combler le vide par une estimation ou une supposition.\n\n"
        f"RÈGLE DE SOURÇAGE DES CHIFFRES :\n"
        f"- Tout chiffre clé (pourcentage, montant, date, taux de croissance, "
        f"volume) doit être repris tel qu'il apparaît dans le contexte, sans "
        f"arrondir ni reformuler la valeur.\n"
        f"- N'invente et n'extrapole jamais un chiffre qui n'est pas présent "
        f"tel quel dans le contexte.\n"
        f"- En cas de chiffres contradictoires entre deux extraits, mentionne "
        f"les deux valeurs et leur source respective plutôt que d'en choisir "
        f"une arbitrairement.\n\n"
        f"STYLE ATTENDU :\n"
        f"- Ton factuel, professionnel, synthétique — style rapport client, "
        f"jamais conversationnel.\n"
        f"- Pas de première personne ('je pense', 'à mon avis').\n"
        f"- Pas de méta-commentaire sur la tâche ou sur le contexte fourni "
        f"('d'après le contexte donné', 'selon les extraits ci-dessus') : "
        f"écris comme un extrait de rapport final, pas comme une réponse à "
        f"une consigne.\n\n"
        f"FORMAT DE SORTIE :\n"
        f"- Réponse rédigée en français, en 6 à 10 phrases, en un ou deux "
        f"paragraphes.\n"
        f"- Pas de titre, pas de liste à puces, sauf si la question porte "
        f"explicitement sur une énumération d'éléments distincts."
    )
        user = (
        ...
        f"## Consigne\n"
        f"Rédige une réponse factuelle, structurée et synthétique (6 à 10 "
        f"phrases) à la question ci-dessus, en t'appuyant strictement sur "
        f"les faits et chiffres présents dans le contexte. Si le contexte "
        f"ne permet pas de répondre complètement, précise-le clairement "
        f"dans ta réponse plutôt que de compléter avec des connaissances "
        f"externes."
        )
        return system, user
